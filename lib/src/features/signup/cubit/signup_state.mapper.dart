...
class SignupStateMapper extends ClassMapperBase<SignupState> {
  ...
  @override
  final MappableFields<SignupState> fields = const {#isLoading: _f$isLoading};

  static SignupState _instantiate(DecodingData data) {
    return SignupState(isLoading: data.dec(_f$isLoading));
  }
...
class _SignupStateCopyWithImpl<$R, $Out>
  ...
  @override
  $R call({bool? isLoading}) =>
      $apply(FieldCopyWithData({if (isLoading != null) #isLoading: isLoading}));
  @override
  SignupState $make(CopyWithData data) =>
      SignupState(isLoading: data.get(#isLoading, or: $value.isLoading));
